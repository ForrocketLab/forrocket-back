...
export class ImportService {
   ...
   * @returns Um array de resultados da importação para cada planilha processada.
   * @throws BadRequestException se o formato do arquivo não for suportado ou se houver erros de processamento.
   */
  async processExcelFile(fileName: string, fileContent: Buffer): Promise<any> {
    ...
    for (const sheetName of orderedSheets) {
      ...
      try {
        ...
        if (records.length === 0) {
          resultsForAllFiles.push({ fileName: currentIdentifier, status: 'skipped', message: `Planilha '${currentIdentifier}' está vazia ou não pôde ser parseada em registros.` });
          continue;
        }
